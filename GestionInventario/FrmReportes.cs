...
namespace GestionInventario
{
    public partial class FrmReportes : Form
    {
        ...
        /*public DataTable ObtenerTendenciaMensual(int mes, int anio)
        {
            ...
            WHERE 
                MONTH(fechaOperacion) = @mes AND YEAR(fechaOperacion) = @anio
            GROUP BY 
                DATE(fechaOperacion)
            ORDER BY 
                Fecha";
        ...
        private void CrearGraficoTendencia(DataTable dataTable)
        {
            ...
            // Agregar el título
            Title title = new Title("Tendencia Mensual de Movimientos", Docking.Top, new Font("Verdana", 12, FontStyle.Bold), Color.Black);
            chartTendencia.Titles.Add(title);
            //Series series = chartTendencia.Series.Add("Tendencia Mensual");
            ...
            // Configurar etiquetas de los puntos de datos
        ...
        private void btnGenerarReporte_Click(object sender, EventArgs e)
        {
            ...
            DataTable tendenciaData = ObtenerTendenciaMensual(mes, año);
            CrearGraficoTendencia(tendenciaData);

            DataTable consumoData = ObtenerConsumoPorTipo(fechaInicio, fechaFin);
            CrearGraficoConsumo(consumoData);
        }
        public DataTable ObtenerConsumoPorTipo(DateTime fechaInicio, DateTime fechaFin)
        {
            DataTable dataTable = new DataTable();
            ...
            FROM 
                salidas_devoluciones 
            WHERE 
                tipoOperacion = 'Traspaso' AND 
                destino = 'Lyfc(traslado)' AND
                estado = 'Activo' AND
        ...
        private void CrearGraficoConsumo(DataTable dataTable)
        {
            ...
            //Series series = chartConsumo.Series.Add("Consumo Cárnico");
            series.ChartType = SeriesChartType.Pie;
            ...
            foreach (DataPoint point in series.Points)
            {
                point.LabelForeColor = Color.Black;
                point.LabelBackColor = Color.White;
                point.LegendText = "#VALX (#PERCENT{P0})";
            }
